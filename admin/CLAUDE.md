...
- **secretaria** — full access to clientes, empleados, and lista de precios (create/edit/delete); also transferencias, planilla-pagarés, contratos, and ventas (registers sales like admin — freely picks the vendedor, doesn't auto-lock to an employee record like a real vendedor does). Can also create/edit vehículos (chasis, specs, precio de venta, etc.) and use the quick price editor, but **never sees or sets `precio_compra`** — see below. Cannot access gastos, usuarios, or flyers, and can't delete vehicles or toggle their web visibility (admin-only).
